...
def create_program(drive_cmd: str, claw_cmd: str) -> str:
    drive_commands = {
        ...
        'adelante_lento': "motorC.run(50)",
        'atras_lento': "motorC.run(-50)",
        'izquierda_lento': "motorA.run(-50)",
        'derecha_lento': "motorA.run(50)",
        'stop': "motorA.stop()\nmotorC.stop()",
    }
    claw_commands = {
...
class LegoGUI:
    ...
    def on_connect(self):
        ...
        self.worker.start()
        # Espera activa: cuando el worker se marque como running, habilita botones
        def check_ready():
