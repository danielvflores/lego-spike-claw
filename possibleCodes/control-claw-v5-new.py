...
if motorE:
    ...
    motorE.run(-150)  # Abrir garra (antihorario)
    wait(500)
    motorE.run(150)   # Cerrar garra (horario)
    wait(500)
    motorE.stop()
...
if motorC:
    motorC.run(300)
print("Motor C running forward at 300")""",
        'atras': """
if motorC:
    motorC.run(-300)
print("Motor C running backward at -300")""",
        'izquierda': """
if motorA:
    motorA.run(-300)
print("Motor A running left at -300")""",
        'derecha': """
if motorA:
    motorA.run(300)
print("Motor A running right at 300")""",
        'izquierda_adelante': """
if motorA:
    motorA.run(-200)
if motorC:
...
print("All drive motors stopped")"""
    ...
    # Comandos de garra (Horario=cerrar, AntiHorario=abrir)
    claw_commands = {
        'cerrar': """
if motorE:
    motorE.run(200)
print("Motor E running to close claw at 200")""",
        'abrir': """
if motorE:
    motorE.run(-200)
print("Motor E running to open claw at -200")""",
        'stop': """
if motorE:
    motorE.stop()
...
if "{drive_command}" != "stop" or "{claw_command}" != "stop":
    # Mantener movimiento por un tiempo para simular movimiento continuo
    for i in range(20):  # 20 iteraciones = ~1 segundo de movimiento
        wait(50)  # 50ms cada iteración
        # Los motores siguen ejecutando el último comando run()
